resource "null_resource" "kube-cni" {
  ...
  provisioner "local-exec" {
    command = "KUBECONFIG=secrets/admin.conf helm install cilium cilium/cilium --version 1.9.1 --namespace kube-system --set prometheus.enabled=true --set devices='{eth0}' --set hostFirewall=false"
  }
